          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                lnurlPayUrl: {
                  type: 'string',
                  description: 'LNURL Pay address to pay',
                },
                amount: {
                  type: 'number',
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        switch (request.params.name) {
          ...
          case 'pay_lnurl_pay_address': {
            const { lnurlPayUrl, amount, comment } = request.params.arguments as { lnurlPayUrl: string, amount: number, comment: string };
            const paymentResult = await this.breezService.payLnurlPayAddress(lnurlPayUrl, amount, comment);
            const message = paymentResult === 'payment_sent' ? 'Payment sent successfully' : 'Payment failed';
            return {
              content: [
                {
                  type: 'text',
